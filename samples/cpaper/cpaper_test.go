...
var _ = Describe(`CommercialPaper`, func() {
	...
	Describe("Commercial Paper lifecycle", func() {
		It("Allow issuer to issue new commercial paper", func() {
			expectcc.ResponseOk(mockstub.Invoke(cpaper.CPaperServiceChaincode_Issue, testdata.Issue1))
		...
		It("Allow issuer to get commercial paper by composite primary key", func() {
			queryResponse := mockstub.Query(cpaper.CPaperServiceChaincode_Get, testdata.Id1)
			...
			Expect(paper.State).To(Equal(cpaper.CommercialPaper_STATE_ISSUED))
		...
		It("Allow issuer to get commercial paper by unique key", func() {
			queryResponse := mockstub.Query(
				cpaper.CPaperServiceChaincode_GetByExternalId, testdata.ExternalId1)
		...
		It("Allow issuer to get a list of commercial papers", func() {
			queryResponse := mockstub.Query(cpaper.CPaperServiceChaincode_List, &empty.Empty{})

			papers := expectcc.PayloadIs(queryResponse, &cpaper.CommercialPaperList{}).(*cpaper.CommercialPaperList)
			...
			Expect(papers.Items[0].State).To(Equal(cpaper.CommercialPaper_STATE_ISSUED))
		...
		It("Allow buyer to buy commercial paper", func() {
			expectcc.ResponseOk(mockstub.Invoke(cpaper.CPaperServiceChaincode_Buy, testdata.Buy1))
			...
			Expect(paper.State).To(Equal(cpaper.CommercialPaper_STATE_TRADING))

		})

		It("Allow buyer to redeem commercial paper", func() {
			expectcc.ResponseOk(mockstub.Invoke(cpaper.CPaperServiceChaincode_Redeem, testdata.Redeem1))
			...
			paper := expectcc.PayloadIs(queryResponse, &cpaper.CommercialPaper{}).(*cpaper.CommercialPaper)
			Expect(paper.Owner).To(Equal(testdata.Issue1.Issuer))
			Expect(paper.State).To(Equal(cpaper.CommercialPaper_STATE_REDEEMED))
		})

		It("Allow issuer to delete commercial paper", func() {
			expectcc.ResponseOk(mockstub.Invoke(cpaper.CPaperServiceChaincode_Delete, testdata.Id1))
	...
	Describe("Commercial Paper Encrypted lifecycle", func() {
		It("Allow issuer to issue new commercial paper", func() {
			...
			expectcc.ResponseOk(mockstubEnc.Invoke(cpaper.CPaperServiceChaincode_Issue, testdata.Issue1))
